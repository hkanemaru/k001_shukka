    ...
    class clsBC
    {
        public static void Save128Image(string sCode, string fileName, ImageFormat imgFormat)
        {
            ...
            try
            {
                ...
                bar.ImgDpi = dpi;
                bar.ImgDrawUnit = unit;

                bar.TextWrite = false; // 添え字
        ...
        public static void SaveQRImage(string sCode, string fileName, ImageFormat imgFormat)
        {
            ...
            float ftxtLineWidht = float.Parse("5");
            try
            {
                Pao.BarCode.QRCode qr = new Pao.BarCode.QRCode(fileName, imgFormat);

                qr.ErrorCorrect = "M"; // 他 L, Q, H エラー訂正レベル
                qr.EncodeMode = "Z"; // 他　N>>数字 , A>>大文字英数字, Z >> 8ビットバイトデータ感じ含む
                qr.Version = 20; // 1 >> 40 まで　規定値 5
                qr.ImgDpi = dpi;
                qr.ImgDrawUnit = unit;
                qr.StringEncoding = "shift-jis"; // utf-8
